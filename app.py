...
st.sidebar.markdown("---")
st.sidebar.caption("🔒 Zero-Knowledge Architecture")

# --- APP MODE: IDENTITY AUDITOR ---
if app_mode == "Identity Auditor":
    st.markdown("<h1>👤 Identity Leak Auditor<br><span style='font-size: 20px;'>Dark Web Credential Monitor</span></h1>", unsafe_allow_html=True)
    st.divider()
...
# --- APP MODE: URL SCANNER ---
    ...
    st.markdown("<h1>🌐 PhishGuard<br><span style='font-size: 20px;'>Hybrid URL Threat Analyzer</span></h1>", unsafe_allow_html=True)
    st.divider()
    ...
    if st.button("🔍 ANALYZE LINK"):
        if url_input:
            ...
            if fast_result["status"] == "malicious":
                st.error(f"🚨 BLOCKED! Detected by {fast_result['source']}")
                st.write(f"**Threat Type:** {fast_result['detail']}")
            elif fast_result["status"] == "error":
                st.warning(f"⚠️ Google API Error: {fast_result.get('message')}")
            else:
